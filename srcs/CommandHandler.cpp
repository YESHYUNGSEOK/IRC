...
void CommandHandler::topic(std::set<Channel *> channels, Client *client, Message message)
{
  // ERR_NEEDMOREPARAMS
  ...
  else
  {
    ...
    if ((*channel_it)->get_mode(TOPIC_RESTRICTED) && !(*channel_it)->is_operator(client))
      return;
...
void CommandHandler::invite(std::set<Channel *> channels, Client *client, Message message)
{
  // ERR_NEEDMOREPARAMS
...
void CommandHandler::kick(std::set<Channel *> channels, Client *client, Message message)
{
  // ERR_NEEDMOREPARAMS
  if (message.get_params().size() < 2)
    return;
