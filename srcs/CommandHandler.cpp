...
void CommandHandler::invite(std::set<Channel *> channels, std::set<Client *> clients, Client *client, Message message)
{
  // ERR_NEEDMOREPARAMS
  ...
  if (!(*channel_it)->is_operator(client) || !(*channel_it)->get_channel_mode(INVITE_ONLY))
    return;
  // ERR_USERONCHANNEL
