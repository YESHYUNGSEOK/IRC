...
//                           Message message) {
//   // ERR_NEEDMOREPARAMS
//   if (!message.get_params().size()) return;

//   std::vector<std::string> depart_channels =
//       split(message.get_params()[0], ',');
//   std::string reason =
//       message.get_params().size() > 1 ? message.get_params()[1] : "";

//   for (std::vector<std::string>::iterator depart_it =
//   depart_channels.begin();
...
//       if ((*channel_it)->get_name() == *depart_it) break;
//     }

//     // ERR_NOSUCHCHANNEL
//     if (channel_it == channels.end()) continue;
//     // ERR_NOTONCHANNEL
//     if (!(*channel_it)->is_client_in_channel(client)) continue;
...
//   std::set<Channel *>::iterator channel_it = channels.begin();
//   for (; channel_it != channels.end(); ++channel_it) {
//     if ((*channel_it)->get_name() == channel_name) break;
//   }

//   // ERR_NOSUCHCHANNEL
//   if (channel_it == channels.end()) return;
//   // ERR_NOTONCHANNEL
//   if (!(*channel_it)->is_client_in_channel(client)) return;
...
//                             Message message) {
//   // ERR_NEEDMOREPARAMS
//   if (message.get_params().size() < 2) return;
...
//     return;
//   // ERR_USERONCHANNEL
//   if ((*channel_it)->is_client_in_channel(client_name)) return;
...
//   if (!(*channel_it)->is_client_in_channel(client)) return;
//   // ERR_CHANOPRIVSNEEDED
//   if (!(*channel_it)->is_operator(client)) return;
