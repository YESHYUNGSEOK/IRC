...
void SocketStream::send()
{
    ...
    std::cout << "[SocketStream] " << _fd << " sent: " << _write_buffer.substr(0, send_len) << std::endl;
...
SocketStream &SocketStream::operator>>(std::string &data)
{
    ...
    std::cout << "[SocketStream] left: " << _read_buffer << std::endl;
    return *this;
}
