...
Channel::Channel(std::string name, std::string key, bool scope, Client *host)
    : _name(name), _key(key), _max_clients(-1), _topic("")
{
  add_client(host);
  add_operator(host);
...
void Channel::set_topic_set_by(const std::string &topic_set_by) { _topic_set_by = topic_set_by; }

void Channel::set_topic_set_time() { _topic_set_time = std::time(NULL); }

void Channel::set_new_topic(const std::string &topic, const std::string &topic_set_by)
{
  set_topic(topic);
