...
bool Server::_sigint_accepted = false;
...
Channel *Server::find_channel_by_name(const std::string &channel_name) {
  ...
  if (it == _channels_by_name.end()) return NULL;
  return it->second;
}
...
void Server::KICK(Client *client, const std::vector<std::string> &params) {
  ...
  targetChannel->kick(client, target_tokens,
                      params.size() >= 3 ? params[2] : "");
}
  ...
  switch (Message::is_valid_mode_flag(params[1])) {
    ...
    case 4: {
      if (params[1][0] == '+') {
        ...
        if (max_users <= 0) {
          *client << ERR_BADCHANMASK_476(*client, params[0]);
          return;
        }
...
void Server::signal_handler(__unused int signum) {
  _sigint_accepted = true;
  DEBUG();
}
...
Server::~Server() {
  std::map<int, Client *>::iterator it = _clients.begin();
  for (; it != _clients.end(); it++) delete it->second;
  std::set<Channel *>::iterator it2 = _channels.begin();
  for (; it2 != _channels.end(); it2++) delete *it2;
  close(_server_fd);
}
