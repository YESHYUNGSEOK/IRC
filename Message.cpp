...
bool Message::parseMsg()
{
	...
	if (std::isalpha(chunk_copy[pos]))
	{
		...
		{
			if (chunk_copy[pos] >= 'a' && chunk_copy[pos] <= 'z')
				return false;
			pos++;
		}
...
const e_cmd &Message::get_command() const {
